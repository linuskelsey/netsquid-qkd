...
5 & RX node loss & $\Delta_n$ & 2.0~dB (lumped; unverified) & \\
6 & Source error rate & $\varepsilon_s$ & 0.015 & Quandela Prometheus datasheet ($g^{(2)}(0)<0.03$) \\
...
Layer~5 introduces receiver node loss as a single lumped parameter $\Delta_n = 2.0$~dB, applied uniformly to both BB84 and MDI-QKD. This captures the insertion loss of optical receiver components (EOM, PBS, HOM coupler) as a single approximate value. Individual component losses are not modelled separately, and the protocol-specific breakdown of these components (which differ between BB84 and MDI receivers) is not reflected in the model. The value is an indicative midpoint pending datasheet verification; this simplification is discussed further in Section~\ref{sec:discussion}. Layer~9 (beam-splitter efficiency) applies to MDI only, where a non-ideal HOM split ratio ($\eta_\text{BS} = 0.97$) reduces coincidence rate. Ideal single-photon sources are used throughout; source imperfection is captured solely by $\varepsilon_s$ at layer~6. Weak coherent pulse (WCP) sources and decoy-state techniques~\cite{Lo_decoys} are not modelled, so reported rates represent an upper bound relative to practical WCP deployments.
...
\begin{itemize}
    ...
    \item Cost--rate optimisation reveals a deployment-optimal relay count for MDI; the optimal $K$ increases with $N$ and area.
