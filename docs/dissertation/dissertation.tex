...
MDI-QKD's elimination of detector side-channel attacks comes at a key rate penalty rooted in its physical architecture. Whereas BB84 requires one photon to traverse the full link and be detected, MDI requires two photons --- one from each party --- to arrive at Charlie's relay simultaneously and produce a coincidence event. Loss on each arm compounds multiplicatively: if the single-arm transmittance is $\eta_\text{arm}$, the BSM detection probability scales as $\eta_\text{arm}^2\,\eta_\text{BS}\,\eta_d^2$, compared to $\eta_\text{link}\,\eta_d$ for BB84. At metropolitan distances and typical detector efficiencies, this effectively squares the channel transmittance, consistent with the order-of-magnitude rate gap between the two protocols in the bipartite setting. This is borne out empirically by the point-to-point sweeps in Section~\ref{sec:results_p2p}: the BB84/MDI ratio stays close to a constant $\sim$5$\times$ under channel-side sweeps (distance, attenuation) but widens to over 25$\times$ under a detector efficiency sweep (Figure~\ref{fig:p2p_efficiency}), the signature of a parameter that acts quadratically on MDI's coincidence probability but only linearly on BB84's single-photon detection probability.
