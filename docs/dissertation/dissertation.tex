...
\begin{itemize}
    ...
    \item \textit{Which topologies favour Weiszfeld.} Weiszfeld does not uniformly beat centroid or boundary placement at the network-key-rate level: across the two-cluster comparison of Section~\ref{sec:relay_placement}, its ranking against the other two strategies varies from seed to seed and from $N$ to $N$, sometimes best, sometimes not. This is expected in principle --- centroid minimises spoke cost per cluster in isolation, while Weiszfeld additionally couples in the backbone term, so the two objectives should diverge most when the backbone is a non-negligible fraction of total fibre length (clusters close together relative to their spread, or markedly unequal in size) and coincide when it is not --- but this work only observed the mixed outcome without identifying which geometric property of a given user configuration predicts it. A natural follow-up would be to regress the per-seed key rate gap between strategies against candidate covariates (cluster size imbalance, inter-cluster separation relative to intra-cluster spread, backbone-to-spoke cost ratio at the initial centroid) using the per-seed data already logged by \texttt{relay\_placement.py}, and, if a clear predictor emerges, confirm it with a dedicated sweep that varies that one covariate while holding the others fixed. Not undertaken here for want of time.
\end{itemize}
