#!/usr/bin/env python3
"""Convert all PDFs in ./artefacts/pdfs to markdown using pymupdf4llm.
...
PDF_DIR = ARTEFACTS / "pdfs"
MD_DIR = ARTEFACTS / "mds"
...
# Suffix characters that follow an author name (superscripts, symbols, letters)
...
def extract_year(text, pdf_path=None):
    ...
    # 7. arXiv filename heuristic: YYMM-NNNNN → 20YY
    if pdf_path:
        m = re.match(r'(\d{2})(\d{2})-\d', pdf_path.name)
        if m:
            return '20' + m.group(1)

    # 8. Fall back: most common plausible year
...
def extract_lead_author(text):
    # Pattern 0: Thesis — "Par" or line-starting "By" (not mid-sentence "by")
    m = re.search(
    ...
    # Pattern 1: "Firstname [Initials] Lastname,digit/symbol"
    # [a-zA-Z'-] handles O'Sullivan, Franco-Rivera but excludes superscript digits
    m = re.search(
        r"(?:^|\n)((?:[A-Z][a-zA-Z'-]*\.?\s+){1,3}"
    ...
    # Pattern 1b: comma-separated list "Firstname Lastname, Firstname2 ..."
    m = re.search(
        r"(?:^|\n)([A-Z]\w+\s+[A-Z]\w+),\s+[A-Z]",
        text[:3000], re.MULTILINE
    )
    ...
    # Pattern 2: "F. Lastname,digit" (initial style, no first name)
    m = re.search(
    ...
    # Pattern 3: "Firstname I.M. Lastname a,b,⇑" (letter affiliation markers)
    m = re.search(
    ...
    # Pattern 4: correspondence author "Firstname Lastname*"
    m = re.search(
        r"(?:^|\n)([A-Z][a-z'-]+\s+[A-Z][a-z'-]+)\s*[*∗⇑]",
        text[:3000], re.MULTILINE
    )
...
def extract_journal(text):
    # DOI-based detection (most reliable)
    ...
    # Text-based fallback (check first 6 pages worth of text)
    sample = text[:8000]
...
def convert_and_rename():
    ...
    if not pdfs:
        print("No PDFs found in ./artefacts/pdfs")
        sys.exit(0)
    ...
    for pdf in pdfs:
        ...
        # Rename any old-style MD that matches this PDF stem
