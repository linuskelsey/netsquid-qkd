...
- [x] **Real-topology input tool**: `network/real_topology.py` — equirectangular lat/lon → km projection; JSON format with `"nodes"` (type: user/relay) and `"protocols"` fields; `load_real_topology()` returns `(Topology, meta)`. `--real NAME` flag on `user_sweep.py` and `cost_sweep.py` loads `data/real_topologies/NAME.json`, fixes relay positions, defaults to a 30 km catchment radius (now CLI-controllable via `--catchment-radius`, generalised from the old hardcoded value), skips k-means. TBB84 gated by `"protocols"` in JSON — only available in `--real` path. BT topology at `data/real_topologies/bt.json` (Slough, West End, City of London; 3 relays; BB84+MDI+TBB84). `relay_sweep.py` excluded (K sweep meaningless with fixed real topology).
  - **[2026-08-10 update]** `user_sweep.py` and `relay_placement.py` (exp 1 + exp 2) rewritten to a provider-growth model: users are drawn incrementally from a single RNG stream (fixes a real bug where the old pool-slice/fresh-per-N draw silently re-rolled an unrelated sample at every N — see `network/topology.py`'s `grow_catchments`), and relay position is placed once (per the chosen strategy: centroid/boundary/weiszfeld, now K-general) and frozen rather than continuously re-optimised. `--catchment-radius` (hard disc) and `--spread` (Gaussian std) both generalised as CLI flags on both scripts. Dissertation §Scalability Analysis and the relay-placement appendix updated to match.
...
- [ ] **[HIGH PRIORITY] Regenerate all figures** — hardware defaults updated (fibre_loss 0.2→0.18, detector_efficiency 0.65→0.90, dark_count_rate 100→50, source_error_rate 0.005→0.015) and node_loss_db unified across protocols. All existing P2P and network figures are stale and must be rerun before thesis submission. *(Plotting infra as of 2026-08-07: every P2P/network script now takes `--output-dir` and writes a `{plot.png, plot.tex, assumptions.md}` bundle — Times New Roman/LaTeX styling, titles cut to ≤6 words, sweep params dumped to assumptions.md instead of cluttering the plot. Ready to run for real now; `scripts/P2P/compare/run_all.py --output-dir ...` sweeps all 11 P2P comparisons in one go.)*
  - **[2026-08-10 update]** `sensitivity.py` and `compare/efficiency.py` re-run, done — see below. Still outstanding under this umbrella: `relay_sweep.py`, `user_sweep.py`, `relay_placement.py` (exp 1 + exp 2, now on the provider-growth model), and the timing scripts (`time_vs_area.py`, `time_vs_users.py`, `adaptive_mc.py`, `surrogate.py`, `trend_analysis.py`).
- [x] **Re-run `sensitivity.py` and `compare/efficiency.py`** (2026-08-09 changes): `sensitivity.py` now has two separate detector-efficiency bars (SPAD η_d=0.20, IDQ ID230 midpoint; SNSPD η_d=0.90) instead of one; `compare/efficiency.py`'s SPAD shaded band corrected from the old InGaAs-SPAD figure (0.15–0.30) to the IDQ ID230-cited range (0.10–0.25) already used elsewhere in the diss, band label updated to "IDQ ID230 SPAD". **Done 2026-08-10**: `efficiency.py` ran without `--output-dir` (plot only shown interactively, no bundle written); reconstructed the `{plot.png, plot.tex, assumptions.md}` bundle straight from the raw per-run rows already committed to `results.db` (same aggregation/plotting code as the script, `--error shade`) rather than re-simulating — saved to `docs/figures/August/final-figs/efficiency/`. `sensitivity.py` re-run live with `--output-dir` pointed at the same folder (21,000 rows: 2 baseline + 9 params×2 protocols + 1 bs_eff MDI-only, ×1000 runtimes) — `docs/figures/August/final-figs/sensitivity/`.
- [ ] **Add $\eta_d=0.1$ point to detector efficiency sweep**: `compare/efficiency.py` currently sweeps `{1, 0.99, 0.95, 0.9, 0.8, 0.7, 0.65, 0.6, 0.5, 0.4, 0.3, 0.2, 0.15}` — stops at 0.15, short of the SPAD shaded band's own lower edge (0.10, IDQ ID230 range). User is running sims now to populate `results.db` with a 0.1 point. Once done, update `figures/p2p_sweeps/efficiency.tex`: extend both BB84/MDI `\addplot` tables and the steelblue/darkorange stats-shading `\path` blocks with the new point, and re-check `xmin`/`ymin` still bound the data.
...
- [x] **Parameter impact summary table**: already existed as Figure~\ref{fig:sensitivity} + prose (§Results, "Parameter sensitivity ranking") rather than a literal table — that prose was stale (written before the 2026-08-09 SPAD/SNSPD split, only described one detector-efficiency bar). **Done 2026-08-10**: reconstructed the exact sensitivity % for all 10 params from the fresh `sensitivity.py` DB rows (21,000 rows, same formula as the script) and rewrote the paragraph to cover both detector bars with current numbers (Fibre loss 64.4/64.7%, SPAD 39.9/48.4%, Node loss 36.9/60.3%, Basis bias 14.1/24.5%, Init loss 10.0/19.2%, SNSPD 5.1/9.8%, BS eff --/6.3%, dark count/source err/dephasing ~0%); also synced the two other paragraphs (§Security–Rate Trade-off, §Cost Model Limitations) that cited the old node-loss/fibre-loss figures. No Charlie-position row — that's a distance-asymmetry sweep, not a fixed-value parameter, and isn't part of `sensitivity.py`'s scope.
