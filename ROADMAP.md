...
| Cost as a function of hardware parameters: current model prices $c_s$/$c_d$/$c_f$ as flat symbolic per-unit constants, independent of the hardware tier actually simulated (e.g. SNSPD vs SPAD, ideal vs realistic source). Extension: tie unit price to the hardware-layer parameters of Table~1 (detector efficiency, dark count rate, source error rate, etc.) via a price-performance relation, so cost and key rate are swept jointly against the same underlying hardware quality axis rather than treated as independent. Would sharpen the dissertation's price-uncertainty discussion (§Cost Model Limitations) into a quantified price-vs-performance trade-off. |
