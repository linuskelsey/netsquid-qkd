"""
Key Rate vs Detector Efficiency Comparison
===========================================
Sweeps detector efficiency (1.0 -> 0.5). Fixed fibre length and loss parameters.
...
if __name__ == "__main__":
    ...
    def _run_sweep(sweep_cfg, label=""):
        ...
        for e in Ex:
            ...
            step += 1
            prog.update(step, f"{tag}Det. eff.: {e}/{Ex[-1]}  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.error in ('bars', 'sigma', 'iqr', 'sem'):
        ...
        ax1.errorbar(Ex, abs_rates_bb84, yerr=yerr_bb84, fmt='o-', label="BB84", capsize=3)
        ax1.errorbar(Ex, abs_rates_mdi,  yerr=yerr_mdi,  fmt='s-', label="MDI",  capsize=3)
    else:
        line1, = ax1.plot(Ex, abs_rates_bb84, 'o-', label="BB84")
        line2, = ax1.plot(Ex, abs_rates_mdi,  's-', label="MDI")
        ...
        ax1.fill_between(Ex, lo1, hi1, alpha=0.15, color=line1.get_color())
        ax1.fill_between(Ex, lo2, hi2, alpha=0.15, color=line2.get_color())
    ax1.set_xlabel("Detector efficiency ratio")
