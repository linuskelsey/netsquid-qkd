...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare-configs", nargs="+", metavar="PATH", dest="compare_configs",
                        help="2–4 config paths; overlay same dark count sweep under each preset on one figure")
    ...
    def _run_sweep(sweep_cfg, label=""):
        ...
        for dc in DCx:
            ...
            step += 1
            prog.update(step, f"{tag}Dark count: {dc}/{DCx[-1]} cps  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.compare_configs:
        ...
        for ci, path in enumerate(cc_paths):
            ...
            c = colors[ci]
            ax1.plot(DCx, [r/1000 for r in res["rates_bb84"]], '--', color=c, lw=1.5, label=f"BB84 - {name}")
            ax1.plot(DCx, [r/1000 for r in res["rates_mdi"]],  '-',  color=c, lw=1.5, label=f"MDI - {name}")
        ax1.set_yscale("log")
        ax1.set_xlabel("Dark count rate (cps)")
        ax1.set_ylabel("Secure key rate (kbps)")
        ax1.legend(fontsize=8)
        ax1.grid(True, alpha=0.3)
        plt.title("Key Rate vs Dark Count Rate (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "dark_count_compare",
                title="Key Rate vs Dark Count Rate (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Dark count sweep points (cps)": DCx,
    ...
    plt.title("Key Rate vs Dark Count Rate")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "dark_count",
            ...
            notes=["Dark count rate is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
