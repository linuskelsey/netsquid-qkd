...
Usage:
    python scripts/P2P/compare/loss.py [--config PATH] [--runtimes N] [--fibre F]
                                       [--det-eff F] [--dark-count N] [--init-loss F]
                                       [--workers N] [--error MODE] [--output-dir DIR]
                                       [--compare-configs PATH [PATH ...]] [--no-plob] [--no-db]

Fixed params default to lib/functions.py DEFAULTS (the layer-9 realistic operating
point: detector_efficiency=0.90, dark_count_rate=50 cps, init_loss=0.10, node_loss_db=2.0 dB,
...
fibre_loss_db_per_km is always the sweep axis, 0-0.3 dB/km — the config value for it
is ignored. PLOB repeaterless bound is overlaid by default; pass --no-plob to suppress it.
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare-configs", nargs="+", metavar="PATH", dest="compare_configs",
                        help="2–4 config paths; overlay same attenuation sweep under each preset on one figure")
    ...
    def _run_sweep(sweep_cfg, label=""):
        ...
        for l in Lx:
            ...
            prog.update(step, f"{tag}Loss: {l}/{Lx[-1]} dB/km  BB84+MDI running...")
            bb84, mdi = main(runtimes=args.runtimes, fibre=args.fibre,
                             lenLoss=l, initLoss=sweep_cfg["init_loss"],
            ...
            step += 1
            prog.update(step, f"{tag}Loss: {l}/{Lx[-1]} dB/km  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.compare_configs:
        ...
        for ci, path in enumerate(cc_paths):
            ...
            c = colors[ci]
            ax1.plot(Lx, [r/1000 for r in res["rates_bb84"]], '--', color=c, lw=1.5, label=f"BB84 - {name}")
            ax1.plot(Lx, [r/1000 for r in res["rates_mdi"]],  '-',  color=c, lw=1.5, label=f"MDI - {name}")
        ax1.set_yscale("log")
        ax1.set_xlabel("Fibre attenuation in dB / km")
        ax1.set_ylabel("Secure key rate (kbps)")
        ax1.legend(fontsize=8)
        ax1.grid(True, alpha=0.3)
        plt.title("Key Rate vs Fibre Loss (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "loss_compare",
                title="Key Rate vs Fibre Loss (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Attenuation sweep points (dB/km)": Lx,
                    "Configs compared": ", ".join(names),
                },
                notes=["BB84 dashed, MDI solid.",
                       "Each config's fixed parameters (detector efficiency, dark count, etc.) come from its own JSON preset; see configs/ for exact values."],
            )
            plt.close()
        else:
            plt.show()
    ...
    if args.plob:
        plob = plob_bound(
            args.fibre,                  # fixed distance (scalar)
            np.array(Lx),               # alpha is the sweep axis
            init_loss    = cfg["init_loss"],
            node_loss_db = cfg["node_loss_db"],
        )
        ax1.plot(Lx, plob, 'k--', lw=1.2, label="PLOB bound", zorder=3)
    ...
    plt.title("Key Rate vs Fibre Loss")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "loss",
            ...
            notes=["Fibre attenuation is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
