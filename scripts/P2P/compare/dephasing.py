...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare-configs", nargs="+", metavar="PATH", dest="compare_configs",
                        help="2–4 config paths; overlay same dephasing sweep under each preset on one figure")
    ...
    def _run_sweep(sweep_cfg, label=""):
        ...
        for dp in Dpx:
            ...
            prog.update(step, f"{tag}Dephasing: {dp}/{Dpx[-1]} /km  BB84+MDI running...")
            bb84, mdi = main(runtimes=args.runtimes, fibre=args.fibre,
                             ...
                             dephasingRate=dp, bsEff=sweep_cfg["bs_eff"],
            ...
            step += 1
            prog.update(step, f"{tag}Dephasing: {dp}/{Dpx[-1]} /km  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.compare_configs:
        ...
        plt.title("Key Rate vs Dephasing Rate (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "dephasing_compare",
                title="Key Rate vs Dephasing Rate (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Dephasing sweep points (/km)": Dpx,
    ...
    if args.error in ('bars', 'sigma', 'iqr', 'sem'):
        ...
        ax1.errorbar(Dpx, abs_rates_bb84, yerr=yerr_bb84, fmt='o-', label="BB84", capsize=3)
        ax1.errorbar(Dpx, abs_rates_mdi,  yerr=yerr_mdi,  fmt='s-', label="MDI",  capsize=3)
    else:
        line1, = ax1.plot(Dpx, abs_rates_bb84, 'o-', label="BB84")
        line2, = ax1.plot(Dpx, abs_rates_mdi,  's-', label="MDI")
        ...
        ax1.fill_between(Dpx, lo1, hi1, alpha=0.15, color=line1.get_color())
        ax1.fill_between(Dpx, lo2, hi2, alpha=0.15, color=line2.get_color())
    ax1.set_xlabel(r"Fibre dephasing rate $\beta$ (per km)")
    ...
    ax2 = ax1.twinx()
    ax2.plot(Dpx, rel_rates_bb84, 'o-', alpha=0)
    ax2.plot(Dpx, rel_rates_mdi,  's-', alpha=0)
    ...
    plt.title("Key Rate vs Dephasing Rate")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "dephasing",
            ...
            notes=["Dephasing rate is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
