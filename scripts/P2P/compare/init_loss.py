"""
Key Rate vs TX Insertion Loss Comparison
=========================================
Sweeps transmitter-side insertion loss (0-0.5 linear fraction). Fixed fibre length,
attenuation, and detector parameters.
Note: init_loss in --config is ignored; L_i is the sweep axis.

Usage:
    python scripts/P2P/compare/init_loss.py [--config PATH] [--runtimes N] [--fibre F]
                                             [--loss F] [--det-eff F] [--dark-count N]
                                             [--node-loss F] [--source-err F] [--workers N]
...
node_loss_db=2.0 dB, source_error_rate=0.015, dephasing_rate=3.2e-7/km, bs_eff=0.97).
Pass --config configs/layer0_ideal.json (or any layerN.json) to override all of them
at once. init_loss is always the sweep axis, 0-0.5 — the config value for it is ignored.
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare-configs", nargs="+", metavar="PATH", dest="compare_configs",
                        help="2–4 config paths; overlay same insertion loss sweep under each preset on one figure")
    ...
    if args.config is not None:
        print(f"Note: init_loss from config ignored — L_i is the sweep axis")
    print(f"Sweep: L_i [0-0.5]  |  Fixed: L={args.fibre} km  α={cfg['fibre_loss_db_per_km']} dB/km  η_d={cfg['detector_efficiency']}")

    ILx = [0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]

    def _run_sweep(sweep_cfg, label=""):
        prog = Progress(len(ILx))
        step = 0
        r_bb84, r_mdi = [], []
        ...
        for il in ILx:
            ...
            prog.update(step, f"{tag}Init loss: {il}/{ILx[-1]}  BB84+MDI running...")
            bb84, mdi = main(runtimes=args.runtimes, fibre=args.fibre,
                             lenLoss=sweep_cfg["fibre_loss_db_per_km"], initLoss=il,
            ...
            step += 1
            prog.update(step, f"{tag}Init loss: {il}/{ILx[-1]}  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.compare_configs:
        ...
        for ci, path in enumerate(cc_paths):
            ...
            c = colors[ci]
            ax1.plot(ILx, [r/1000 for r in res["rates_bb84"]], '--', color=c, lw=1.5, label=f"BB84 - {name}")
            ax1.plot(ILx, [r/1000 for r in res["rates_mdi"]],  '-',  color=c, lw=1.5, label=f"MDI - {name}")
        ax1.set_yscale("log")
        ax1.set_xlabel("TX insertion loss $L_i$ (linear fraction)")
        ax1.set_ylabel("Secure key rate (kbps)")
        ax1.legend(fontsize=8)
        ax1.grid(True, alpha=0.3)
        plt.title("Key Rate vs Init Loss (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "init_loss_compare",
                title="Key Rate vs Init Loss (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Init loss sweep points": ILx,
    ...
    if args.error in ('bars', 'sigma', 'iqr', 'sem'):
        ...
        ax1.errorbar(ILx, abs_rates_bb84, yerr=yerr_bb84, fmt='o-', label="BB84", capsize=3)
        ax1.errorbar(ILx, abs_rates_mdi,  yerr=yerr_mdi,  fmt='s-', label="MDI",  capsize=3)
    else:
        line1, = ax1.plot(ILx, abs_rates_bb84, 'o-', label="BB84")
        line2, = ax1.plot(ILx, abs_rates_mdi,  's-', label="MDI")
    ...
    ax2 = ax1.twinx()
    ax2.plot(ILx, rel_rates_bb84, 'o-', alpha=0)
    ax2.plot(ILx, rel_rates_mdi,  's-', alpha=0)
    ax2.set_ylabel("Relative secure key rate")
    ax2.set_yscale("log")

    ax1.legend()

    REALISTIC_MIN, REALISTIC_MAX = 0.05, 0.20
    ...
    for _xv, _lbl, _ha in [(REALISTIC_MIN, "0.05", _ha_min), (REALISTIC_MAX, "0.20", _ha_max)]:
    ...
    plt.title("Key Rate vs Init Loss")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "init_loss",
            ...
            notes=["TX insertion loss is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
