...
if __name__ == "__main__":
    ...
    if args.compare_configs:
        ...
        plt.title("Key Rate vs Source Error (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "source_err_compare",
                title="Key Rate vs Source Error (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Source error sweep points": SEx,
    ...
    if args.error in ('bars', 'sigma', 'iqr', 'sem'):
        ...
        ax1.errorbar(SEx, abs_rates_bb84, yerr=yerr_bb84, fmt='o-', label="BB84", capsize=3)
        ax1.errorbar(SEx, abs_rates_mdi,  yerr=yerr_mdi,  fmt='s-', label="MDI",  capsize=3)
    else:
        line1, = ax1.plot(SEx, abs_rates_bb84, 'o-', label="BB84")
        line2, = ax1.plot(SEx, abs_rates_mdi,  's-', label="MDI")
        ...
        ax1.fill_between(SEx, lo1, hi1, alpha=0.15, color=line1.get_color())
        ax1.fill_between(SEx, lo2, hi2, alpha=0.15, color=line2.get_color())
    ax1.set_xlabel(r"Source error rate $\varepsilon_s$")
    ...
    plt.title("Key Rate vs Source Error")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "source_err",
            ...
            notes=["Source error rate is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
