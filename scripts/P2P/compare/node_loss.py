...
if __name__ == "__main__":
    ...
    parser.add_argument("--compare-configs", nargs="+", metavar="PATH", dest="compare_configs",
                        help="2–4 config paths; overlay same node loss sweep under each preset on one figure")
    ...
    def _run_sweep(sweep_cfg, label=""):
        ...
        for nl in NLx:
            ...
            prog.update(step, f"{tag}Node loss: {nl}/{NLx[-1]} dB  BB84+MDI running...")
            bb84, mdi = main(runtimes=args.runtimes, fibre=args.fibre,
                             ...
                             nodeLossDb=nl, sourceErrRate=sweep_cfg["source_error_rate"],
            ...
            step += 1
            prog.update(step, f"{tag}Node loss: {nl}/{NLx[-1]} dB  BB84 {bb84[3]/1000:.2f} | MDI {mdi[3]/1000:.2f} kbps")
        prog.stop()
        return dict(
    ...
    if args.compare_configs:
        ...
        for ci, path in enumerate(cc_paths):
            ...
            c = colors[ci]
            ax1.plot(NLx, [r/1000 for r in res["rates_bb84"]], '--', color=c, lw=1.5, label=f"BB84 - {name}")
            ax1.plot(NLx, [r/1000 for r in res["rates_mdi"]],  '-',  color=c, lw=1.5, label=f"MDI - {name}")
        ax1.set_yscale("log")
        ax1.set_xlabel("Node/connector loss (dB)")
        ax1.set_ylabel("Secure key rate (kbps)")
        ax1.legend(fontsize=8)
        ax1.grid(True, alpha=0.3)
        plt.title("Key Rate vs Node Loss (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "node_loss_compare",
                title="Key Rate vs Node Loss (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "Node loss sweep points (dB)": NLx,
    ...
    if args.plob:
        plob = plob_bound(
            ...
            node_loss_db = np.array(NLx),
        )
        ax1.plot(NLx, plob, 'k--', lw=1.2, label="PLOB bound", zorder=3)
    ...
    plt.title("Key Rate vs Node Loss")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "node_loss",
            title="Key Rate vs Node Loss",
            assumptions={
                "Fibre length": f"{args.fibre} km",
            ...
            notes=["Node/connector loss is the swept axis; config value for it is ignored."],
        )
        plt.close()
    else:
        plt.show()
