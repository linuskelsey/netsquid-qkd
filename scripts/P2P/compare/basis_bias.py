...
if __name__ == "__main__":
    ...
    if args.compare_configs:
        ...
        plt.title("Key Rate vs Basis Bias (Comparison)")
        plt.tight_layout()
        if args.output_dir:
            save_bundle(
                fig, args.output_dir, "basis_bias_compare",
                title="Key Rate vs Basis Bias (Comparison)",
                assumptions={
                    "Fibre length": f"{args.fibre} km",
                    "Runtimes per point": args.runtimes,
                    "Error display": args.error,
                    "X-basis sweep points": Bx,
    ...
    plt.title("Key Rate vs Basis Bias")
    if args.output_dir:
        save_bundle(
            fig, args.output_dir, "basis_bias",
            title="Key Rate vs Basis Bias",
            assumptions={
                "Z-basis detector efficiency (fixed)": det_eff_z,
                "Fibre length": f"{args.fibre} km",
            ...
            notes=["X-basis detector efficiency eta_X is the swept axis; eta_Z is held fixed."],
        )
        plt.close()
    else:
        plt.show()
