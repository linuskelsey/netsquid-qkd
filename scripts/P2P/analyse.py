...
def _fixed_str(row_meta, sweep_col):
    """Build title fixed-params string, excluding the sweep column."""
...
def query_and_aggregate(conn, script, sweep_col):
    ...
    cols = ("fibre_len", "fibre_loss", "det_eff", "dark_count",
            "init_loss", "node_loss", "source_err", "dephasing", "bs_eff")
    col_str = ", ".join(cols)
    rows = conn.execute(
        f"SELECT protocol, {sweep_col}, key_rates, {col_str} "
    ...
    for protocol in protocols:
        ...
        results[protocol] = (xs, avgs, mins, maxs, stds)
...
def plot_results(results, x_label, title, fixed, error_style, invert_x, rmin, rmax, region_label):
    ...
    for protocol, (xs, avgs, mins, maxs, stds) in results.items():
